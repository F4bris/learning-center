...
const usePublishingStore = defineStore('publishing', () => {
    ...
    const categoriesCount = computed(() => {
        return categoriesLoaded ? categories.value.lenght : 0 ;
    });
    ...
    function updateCategory(category){
        publishingApi.updateCategory(category).then((response) => {
            ...
            const index = category.value.findIndex(c => c['id'] === resource.id);
    ...
    function deleteCategory(category){
        publishingApi.deleteCategory(category).then((response) => {
